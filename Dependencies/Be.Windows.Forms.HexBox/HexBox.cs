...
namespace Be.Windows.Forms
{
	...
	public class HexBox : Control
	{
		...
		static HexBox()
		{
			_hexLookup = new string[ byte.MaxValue ];
			_asciiLookup = new string[ byte.MaxValue ];
		...
		void PaintLineInfo(Graphics g, long startByte, long endByte)
		{
			...
			if( lineInfoBrush == null )
			{
				Color lineInfoColor = ( this.LineInfoForeColor != Color.Empty ) ? this.LineInfoForeColor : this.ForeColor;
				lineInfoBrush = new SolidBrush( lineInfoColor );
			}
			
			int maxLine = GetGridBytePoint(endByte-startByte).Y+1;
			for(int i = 0; i < maxLine; i++)
			{
				long firstLineByte = startByte + (_iHexMaxHBytes)*i + _byteProvider.Offset;
				PointF bytePointF = GetBytePointF(new Point(0, 0+i));
				string formattedInfo = string.Format( "{0,-8:X8}", firstLineByte );
				g.DrawString( formattedInfo, Font, lineInfoBrush, new PointF( _recLineInfo.X, bytePointF.Y ), _stringFormat );
			}
		}

		private Brush hexBrush;
		private Brush selectedHexBrush;
		private Brush selectedHexBrushBack;

		void PaintHex(Graphics g, long startByte, long endByte)
		{
			if( hexBrush == null )
			{
				hexBrush = new SolidBrush( GetDefaultForeColor() );
				selectedHexBrush = new SolidBrush( _selectionForeColor );
				selectedHexBrushBack = new SolidBrush( _selectionBackColor );
			}
			...
			for(long i = startByte; i < intern_endByte+1; i++)
			{
				...
				{
					PaintHexStringSelected( g, b, selectedHexBrush, selectedHexBrushBack, gridPoint );
				}
				else
				{
					PaintHexString( g, b, hexBrush, gridPoint );
				}
			}
		}
		...
		void PaintHexAndStringView(Graphics g, long startByte, long endByte)
		{
			...
			for(long i = startByte; i < intern_endByte+1; i++)
			{
				...
				string s = _asciiLookup[ b ];
				if(isSelectedByte && isStringKeyInterpreterActive)
				{
					g.FillRectangle( selectedHexBrushBack, byteStringPointF.X, byteStringPointF.Y, _charSize.Width, _charSize.Height );
					g.DrawString( s, Font, selectedHexBrush, byteStringPointF, _stringFormat );
				}
				else
				{
					g.DrawString( s, Font, hexBrush, byteStringPointF, _stringFormat );
				}
			}
		}

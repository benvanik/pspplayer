			...
			for( int n = 0; n < list.Packets.Count; n++ )
			{
				...
				switch( packet.Command )
				{
					...
					case VideoCommand.CLEAR:
						...
						if( ( packet.Argument & 0x1 ) == 0x1 )
						{
							...
							if( ( packet.Argument & 0x200 ) != 0 )
							{
								...
								//if( clearAny == true )
								//    clearFlags |= ClearOptions.Stencil;
								//else
								//    clearFlags = ClearOptions.Stencil;
								//clearAny = true;
							}
							if( ( packet.Argument & 0x400 ) != 0 )
							{
								// Clear depth buffer
								//if( clearAny == true )
								//    clearFlags |= ClearOptions.ZBuffer;
								//else
								//    clearFlags = ClearOptions.ZBuffer;
								//clearAny = true;
							}
							// Done at start of the next frame
							_context.ClearOptions = clearOptions;
						}
						break;
					...
					case VideoCommand.BCE:
						_device.RenderState.CullMode = ( packet.Argument == 0 ? CullMode.None : _device.RenderState.CullMode );
						break;
